...
class HelpCommand(commands.MinimalHelpCommand):
    ...
    @classmethod
    async def filter_commands_classmethod(
        cls, ctx, commands_to_filter: typing.List[commands.Command]
        ...
        if ctx.author.id in ctx.bot.owner_ids:
            return [
                i for i in commands_to_filter if i.name not in cls.HELP_COMMAND_NAMES
            ]
        valid_commands = [
            i
            for i in commands_to_filter
            if i.hidden is False
            and i.enabled is True
            and i.name not in cls.HELP_COMMAND_NAMES
        ]
        returned_commands = []
    ...
    async def filter_commands(
        self, commands_to_filter: typing.List[commands.Command]
    ) -> typing.List[commands.Command]:
        """
        Filter the command list down into a list of runnable commands.
        """
    ...
    def get_command_signature(self, command: commands.Command):
        return "{0.context.clean_prefix}{1.qualified_name} {1.signature}".format(
            self, command
        )
    ...
    async def send_cog_help(self, cog: commands.Cog):
        ...
        return await self.send_bot_help({cog: cog.get_commands()})
    ...
    async def send_group_help(self, group: commands.Group):
        ...
        return await self.send_bot_help({group: group.commands})
    ...
    async def send_bot_help(
        ...
        for cog, cog_commands in mapping.items():
            ...
            if len(available_commands) > 0 or isinstance(
                cog,
                (
                    commands.Command,
                    commands.Group,
        ...
        for cog, cog_commands in runnable_commands.items():
            ...
            if isinstance(cog, commands.Group):
                help_embed.description = self.get_help_line(
                    cog, with_signature=cog.invoke_without_command
                )
        ...
        command_strings = sorted(
            list(command_string_dict.items()),
            ...
            data.update(
                {
                    "content": content.format(
                        bot=self.context.bot, prefix=self.context.clean_prefix
                    )
                }
            )
        ...
        if isinstance(dest, (discord.User, discord.Member, discord.DMChannel)):
            try:
                ...
                await self.context.send(
                    f"I couldn't send you the help DM - {e}"
                )  # We couldn't send the embed for some other reason
            return
    ...
    def get_destination(self):
        ...
        if (
            self.context.invoked_with.lower() == "channelhelp"
            and self.context.channel.permissions_for(
                self.context.author
            ).manage_messages
        ):
