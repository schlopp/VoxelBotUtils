...
def create_file(
    *path: str, content: typing.Optional[typing.Union[str, pathlib.Path]] = None
):
...
def get_default_program_arguments() -> argparse.ArgumentParser:
    ...
    website_subparser.add_argument(
        ...
        nargs="?",
        default=".",
        help="The directory containing a static and templates folder for the website to run.",
    )
    website_subparser.add_argument(
        "config_file",
        nargs="?",
        default="config/website.toml",
        help="The configuration for the website.",
    )
    website_subparser.add_argument(
        "--host",
        nargs="?",
        default="0.0.0.0",
        help="The host IP to run the website on.",
    )
    website_subparser.add_argument(
        "--port",
        nargs="?",
        type=int,
        default="8080",
        help="The port to run the website with.",
    )
    website_subparser.add_argument(
        "--debug",
        action="store_true",
        default=False,
        help="Whether or not to run the website in debug mode.",
    )
    website_subparser.add_argument(
        "--loglevel",
        nargs="?",
    ...
    interactions_subparser.add_argument(
        "bot_directory",
        nargs="?",
        default=".",
        help="The directory containing a config and a cogs folder for the bot to run.",
    ...
    interactions_subparser.add_argument(
        ...
        help="The path to run the interactions endpoint on.",
    )
    interactions_subparser.add_argument(
        "--connect",
        action="store_true",
        default=False,
        help="Whether you want your bot to connect to the Discord gateway.",
    )
    interactions_subparser.add_argument(
        "--debug",
        action="store_true",
    ...
    sharder_subparser.add_argument(
        "config_file",
        nargs="?",
        default="config/config.toml",
        help="The configuration for the bot.",
    ...
    sharder_subparser.add_argument(
        "--host", nargs="?", default="127.0.0.1", help="The host address to listen on."
    )
    sharder_subparser.add_argument(
        "--port", nargs="?", default=8888, type=int, help="The host port to listen on."
    )
    sharder_subparser.add_argument(
        "--concurrency",
        nargs="?",
        default=1,
        type=int,
        help="The max concurrency of the connecting bot.",
    )
    sharder_subparser.add_argument(
        "--loglevel",
        nargs="?",
    ...
    # Set up the shell arguments
    application_subparser.add_argument(
        "action",
        help="The action yuou want to take on your application commands.",
        choices=["add", "remove"],
    )
    application_subparser.add_argument(
        "bot_directory",
    ...
    application_subparser.add_argument(
        ...
        default=None,
        type=int,
        help="The guild that you want to modify the application commands for.",
    )
    ...
    create_config_subparser.add_argument(
        "config_type",
        nargs=1,
        help="The type of config file that we want to create.",
        choices=["bot", "website", "all"],
    )
    check_config_subparser.add_argument(
        "config_type",
        nargs=1,
        help="The type of config file that we want to create.",
        choices=["bot", "website"],
    ...
    check_config_subparser.add_argument(
        ...
        nargs="?",
        default="config/config.toml",
        help="The configuration file that you want to check.",
    )
...
def check_config_value(
    base_config_key: typing.List[str],
    base_config_value: typing.Any,
    compare_config_value: typing.Any,
) -> None:
    """
    ...
    default_value = (
        ...
        else textwrap.dedent(base_config_value).replace("\n", "\\n")
    )

    # See if the item was omitted
    if isinstance(compare_config_value, type(None)):
        print(
            f"No value {base_config_key} was provided in your config file - should be type {type(base_config_value).__name__} (default {default_value!r})."
        )
    ...
    if not isinstance(base_config_value, type(compare_config_value)):
        print(
            f"Wrong value {base_config_key} type was provided in your config file - should be type {type(base_config_value).__name__} (default {default_value!r})."
        )
        if isinstance(base_config_value, dict):
            for i, o in base_config_value.items():
                check_config_value(base_config_key + [i], o, None)
...
def get_next_version(current_version) -> str:
    ...
    release, major, minor = [
        int("".join(o for o in i if o.isdigit())) for i in current_version.split(".")
    ]
    if release == 0:
...
def main():
    ...
    if args.subcommand == "create-config":
        ...
        if config_type in ["website", "all"]:
            create_file(
                "config",
                "website.toml",
                content=get_path_relative_to_file(
                    "config/web_config_example_file.toml"
                ),
            )
            create_file(
                "config",
            ...
            create_file(
                "config",
                "database.pgsql",
                content=get_path_relative_to_file("config/database_base_file.pgsql"),
            )
            ...
            create_file(
                "requirements.txt",
                content=f"voxelbotutils[web]>={__version__},<{next_version}\n",
            )
            create_file(
                "website",
                "frontend.py",
                content=get_path_relative_to_file("config/website_frontend_content.py"),
            )
            create_file(
                "website",
                "backend.py",
                content=get_path_relative_to_file("config/website_backend_content.py"),
            )
        ...
        if config_type in ["bot", "all"]:
            create_file(
                "config",
                "config.toml",
                content=get_path_relative_to_file("config/config_example_file.toml"),
            )
            create_file(
                "config",
                "config.example.toml",
            ...
            create_file(
                "cogs",
                "ping_command.py",
                content=get_path_relative_to_file("config/cog_example_file.py"),
            )
            ...
            create_file(
                "requirements.txt",
                content=f"voxelbotutils>={__version__},<{next_version}\n",
            )
            create_file(".venv")
    ...
    # See if we want to check the config file
        ...
        if config_type == "website":
            with open(
                get_path_relative_to_file("config/web_config_example_file.toml")
            ) as a:
                base_config_file_text = a.read()
        elif config_type == "bot":
            with open(
                get_path_relative_to_file("config/config_example_file.toml")
            ) as a:
                base_config_file_text = a.read()
        else:
            exit(1)
