...
class Bot(MinimalBot):
    ...
    async def _startup(self):
        ...
        default_guild_settings = await db(
        ...
        if not default_user_settings:
            ...
            default_user_settings = await db(
    ...
    async def _get_all_table_data(self, db, table_name):
        ...
        return await self._run_sql_exit_on_error(
            db, "SELECT * FROM {0}".format(table_name)
        )
    ...
    async def add_delete_reaction(
        ...
        *,
        ...
        bulk = False
        if message.guild:
            permissions: discord.Permissions = message.channel.permissions_for(
                message.guild.me
            )
            bulk = permissions.manage_messages and permissions.read_message_history
        try:
            await message.channel.purge(
                check=lambda m: m.id in [i.id for i in delete], bulk=bulk
            )
        except Exception:
            return  # Ah well
    ...
    async def start(self, token: str = None, *args, **kwargs):
        ...
        try:
            ...
            self.logger.info(
                f"Recommended shard count for this bot: {recommended_shard_count}"
            )
            self.logger.info(
            ...
            self.logger.info(
                "Recommended shard count for this bot could not be retrieved"
            )
        else:
