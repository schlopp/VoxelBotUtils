...
def get_discord_login_url(request: Request, redirect_uri: str | None = None) -> str:
    ...
    if redirect_uri:
        if "http" not in redirect_uri:
            redirect_uri = (
                config["website_base_url"].rstrip("/") + "/" + redirect_uri.lstrip("/")
            )
...
async def process_discord_login(request: Request) -> None:
    ...
    if base_url.explicit_port:
        data["redirect_uri"] = "{0.scheme}://{0.host}:{0.port}{1.path}".format(
            base_url, request.url
        )
    else:
        data["redirect_uri"] = "{0.scheme}://{0.host}{1.path}".format(
            base_url, request.url
        )
    ...
    async with aiohttp.ClientSession(loop=request.loop) as session:
        ...
        # Update headers
        headers.update({"Authorization": f"Bearer {token_info['access_token']}"})
        token_info["expires_at"] = (
            dt.utcnow() + timedelta(seconds=token_info["expires_in"])
        ).timestamp()
...
async def get_access_token_from_session(
    request: Request, *, refresh_if_expired: bool = True, refresh: bool = False
) -> str:
    """
    ...
    if refresh:
        ...
        refresh_if_expired is False
        or session_storage["token_info"]["expires_at"] < dt.utcnow().timestamp()
    ):
...
async def get_user_guilds_from_session(
    request: Request, bot_key: str = "bot"
) -> typing.List[OauthMember]:
    """
...
async def add_user_to_guild_from_session(
    request: Request, bot_index: str, guild_id: int
) -> bool:
    """
    ...
    # Get our headers
    guild_join_url = (
        f"https://discordapp.com/api/v6/guilds/{guild_id}/members/{user_info['id']}"
    )
    headers = {
    ...
    data = {"access_token": await get_access_token_from_session(request)}
